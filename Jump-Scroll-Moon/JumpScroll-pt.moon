export script_name = 'SaltaScroll'
export script_description = 'Guarda/Carrega posição da legenda (Funciona apenas em Windows)'
...
update_statusbar = (i, saved=true) ->
    if h.Statusbar
        ffi.C.SendMessageA h.Statusbar, msgs.SB_SETTEXT, 0,
            ffi.cast 'uintptr_t',
                ('SaltaScroll #%d: posição %s %d')\format i, saved and 'guardada em' or 'do scroll em', jumpscroll[i]
...
for i=1,jumpscroll_max
    aegisub.register_macro script_name..'/Guardar/'..i,
        'Guardar posição da legenda em #'..i,
        -> save_scroll_pos i

for i=1,jumpscroll_max
    aegisub.register_macro script_name..'/Carregar/'..i,
        'Ir para a posição da legenda guardada em #'..i,
        -> load_scroll_pos i